...
tpcs=matrix(nrow=length(text),ncol=length(topic_list$V1))

tpcs<-data.frame(tpcs)

colnames(tpcs)=topic_list$V1
...
for (i in 1:length(topic_list$V1))
{
  ...
  for(j in 1:length(found))
  {
      ...
      tpcs[found[j],i]=as.character(topic_list[i,1])
...
MH_Extract<-data.frame(MH_Extract,Sentiment=sentiment,tpcs)

write.csv(MH_Extract,file=paste(length(topic_list$V1)," Topics and Sentiments.csv"))
